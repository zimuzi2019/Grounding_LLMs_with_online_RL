...
class Level_MixedLocal(LevelGen):
    """
    ...
    The original Level_MixedTestLocal only give to the agent the instructions containing:
    - yellow box
    - red door/key
    - green ball
    - grey door
    ...
    Without fine-tuning the model, we don't need this kind of restriction.
    ...
    def __init__(
        ...
        if action is None:
            ...
            assert action in ['goto', 'pickup', 'open', 'putnext', 'pick up seq go to']
        
        if action == 'open':
            num_cols = 2
            num_rows = 1
        # Add many distractors to increase the probability of ambiguous locations within the same room.
        super().__init__(
            room_size=room_size,
            num_rows=num_rows,
            num_cols=num_cols,
    ...
    def _regen_grid(self):
        ...
        for j in range(0, self.num_rows):
            # For each column of rooms
            for i in range(0, self.num_cols):
                room = self.room_grid[j][i]  # Current room

                x_l, y_l = (room.top[0] + 1, room.top[1] + 1) # Left-top corner coordinates
                x_m, y_m = (room.top[0] + room.size[0] - 1, room.top[1] + room.size[1] - 1) # Right-bottom corner coordinates

                # Door positions, order is right, down, left, up
                if i < self.num_cols - 1: # if room is not on the right edge, add door on the right
                    room.neighbors[0] = self.room_grid[j][i+1]  
                    room.door_pos[0] = (x_m, self._rand_int(y_l, y_m))
                if j < self.num_rows - 1: # if room is not on the bottom edge, add door on the bottom
                    room.neighbors[1] = self.room_grid[j+1][i]
                    room.door_pos[1] = (self._rand_int(x_l, x_m), y_m)
                if i > 0:                 # if room is not on the left edge, set door on the left with the same position as the right door of the left room
                    room.neighbors[2] = self.room_grid[j][i-1]
                    room.door_pos[2] = room.neighbors[2].door_pos[0]
                if j > 0:                 # if room is not on the top edge, set door on the top with the same position as the bottom door of the top room
